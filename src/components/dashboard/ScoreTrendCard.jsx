import { cn, eyebrow, metricBlock, metricLabel, metricValue, panelCard, panelChip, panelHeading, panelTitle } from '../../lib/ui'
...
export function ScoreTrendCard({ chartPoints, weekdayHighlights }) {
  ...
  const selectedHighlight =
    weekdayHighlights.find((day) => day.day === selectedDay) ?? weekdayHighlights.at(-1)
  const averageScore = Math.round(
    weekdayHighlights.reduce((sum, day) => sum + day.score, 0) / weekdayHighlights.length,
  )
  ...
  return (
    <section className={panelCard}>
      ...
      <div className="rounded-[24px] border border-[color:var(--line)] bg-[color:var(--bg-panel)] p-4">
        <svg viewBox="0 0 336 140" aria-label="Weekly score line chart" role="img">
          <defs>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
        {weekdayHighlights.map((day) => (
          ...
          >
            ...
            <strong className="mt-2 block text-xl font-bold">{day.score}</strong>
          </button>
        ))}
      </div>
      <div className="mt-5 flex flex-col gap-4 rounded-[24px] border border-[color:var(--line)] bg-[color:var(--bg-panel)] p-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <strong className="text-2xl font-bold text-[color:var(--text)]">{selectedHighlight.day}</strong>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <div className={metricBlock}>
            <span className={metricLabel}>Interaction score</span>
            <strong className={metricValue}>{selectedHighlight.score}</strong>
          </div>
          <div className={metricBlock}>
